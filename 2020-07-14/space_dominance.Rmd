...
missions_by_year_country %>% filter(nationality %in% top_countries$nationality) %>% 
  ...
  labs(title = 'Missions by Year', subtitle = 'for Top Countries per pilot') +
...
astronauts %>% group_by(nationality) %>% summarise(hours = mean(total_hrs_sum), eva = mean(total_eva_hrs)) %>% arrange(desc(hours)) %>% head(10) %>% 
  ...
  labs(title = "Average Mission Hours Per Astronaut By Country", subtitle = "shaded by EVA hours.") +
...
# Findings
It seems that overall the U.S. wins the Space race when inspecting raw mission hours and succesful launches.
On average the U.S. has more launches, launch craft and pilots while maintaining a reasonable average expedition hours count.
